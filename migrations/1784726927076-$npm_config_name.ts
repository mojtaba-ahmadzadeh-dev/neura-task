...
export class CreateProjectsTable1784726927076 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    if (projectsTable) {
      ...
      // حذف ایندکس
      const titleIndex = projectsTable.indices.find((index) => index.name === 'IDX_projects_title');
      if (titleIndex) {
        await queryRunner.dropIndex('projects', titleIndex);
      }

      // حذف ستون workspaceId (اگه جدا از جدول اصلی اضافه شده بود)
      const hasWorkspaceIdColumn = projectsTable.columns.some((col) => col.name === 'workspaceId');
